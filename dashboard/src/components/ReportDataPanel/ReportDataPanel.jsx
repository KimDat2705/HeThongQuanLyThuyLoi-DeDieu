/* ============================================
   REPORT DATA PANEL — Accordion Layout v2
   Sửa lỗi scroll + Tách Hồ chứa / Máy bơm
   ============================================ */
...
function AccordionItem({ id, icon: Icon, title, badge, badgeType, isOpen, onToggle, children }) {
  return (
    <div className={styles.accordion}>
      <button className={styles.accHeader} onClick={() => onToggle(id)}>
        ...
        <span className={`${styles.accBadge} ${
          badgeType === 'warn' ? styles.accBadgeWarn :
          badgeType === 'safe' ? styles.accBadgeSafe : ''
        }`}>{badge}</span>
  ...
  return (
    <aside className={styles.panel} id="report-data-panel">
      ...
      <div className={styles.panelScroll}>
        ...
        {/* ===== 1. BÃO SỐ 11 ===== */}
        <AccordionItem
          id="storm" icon={FiWind} title="Bão số 11"
          badge={`Cấp ${stormData.capBao} • Giật ${stormData.giatCap}`}
        ...
        <AccordionItem
          ...
          badge={`TB ${rainfallData.trungBinh}mm • Max ${rainfallData.caoNhat.value}mm`}
        ...
        <AccordionItem
          ...
          badge={`${aboveBD1}/${allStations.length} trạm ≥BĐ1`}
          badgeType={aboveBD1 > 0 ? 'warn' : 'safe'}
        ...
        <AccordionItem
          ...
          badge={`${reservoirData.minPercent}–${reservoirData.maxPercent}%`}
        ...
        {/* ===== 5. MÁY BƠM TIÊU (tách riêng) ===== */}
        <AccordionItem
          id="pump" icon={FiActivity} title="Máy bơm tiêu"
          badge={`${pumpData.total} máy`}
        ...
        {/* ===== 6. SỰ CỐ CÔNG TRÌNH ===== */}
        <AccordionItem
          id="incident" icon={FiShield} title="Sự cố công trình"
          badge={`${totalSuCo} sự cố`} badgeType="warn"
        ...
        {/* ===== 7. THIỆT HẠI ===== */}
        <AccordionItem
          id="damage" icon={FiCheckCircle} title="Thiệt hại"
          badge="Không phát sinh" badgeType="safe"
